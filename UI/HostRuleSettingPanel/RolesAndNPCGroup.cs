...
public class RolesAndNPCGroup : MonoBehaviour
{
    ...
    [SerializeField] private RectTransform npcRoleOptionParent = null;
    ...
    private RoleOption[] npcRoleOptionList = null;
    ...
    public PhotonHashTable OptionSetting
    {
        get
        {
            ...
            // NPC Roles
            List<int> npcRoles = new();
            for (int index = 0; index < npcRoleOptionList.Length; index++)
            {
                if (npcRoleOptionList[index].OptionValue == RoleOption.EOptionState.ESSENTIAL)
                {
                    npcRoles.Add(index);
                }
            }
            optionSetting.Add(CustomProperties.NPC_ROLES, ArrayHelper.ListToArray(npcRoles));
    ...
    public void InitGroup()
    {
        ...
        List<RoleOption> npcRoleOptionList = new();
        RoleInfo[] npcRoles = roleData.GetAllRoleInfos(ERoleType.NPC);
        foreach (RoleInfo roleInfo in npcRoles)
        {
            RoleOption roleOption = Instantiate(roleGroupPrefab, npcRoleOptionParent).GetComponent<RoleOption>();
            roleOption.InitOption(roleInfo, true);
            npcRoleOptionList.Add(roleOption);
        }
        this.npcRoleOptionList = ArrayHelper.ListToArray(npcRoleOptionList);
    ...
    public void SetGroup(PhotonHashTable roomSetting)
    {
        ...
        // Set the option state of NPC roles
        roleList = (int[])roomSetting[CustomProperties.NPC_ROLES];
        foreach(int role in roleList)
        {
            npcRoleOptionList[role].OptionValue = RoleOption.EOptionState.ESSENTIAL;
        }
    }
